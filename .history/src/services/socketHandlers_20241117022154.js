...
const socketHandlers = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('likePublication', async ({ publicationId, userId }) => {
            try {
                ...
                if (publication) {
                    ...
                    const userIndex = publication.likes.indexOf(userId);
                    if (userIndex === -1) {
                        // Si el usuario no ha dado like, agregar su ID al array de likes
                        publication.likes.push(userId);
                    } else {
                        // Si el usuario ya ha dado like, quitar su ID del array de likes
                        publication.likes.splice(userIndex, 1);
                    }
                    await publication.save();
...
socket.on('addComment', async ({ publicationId, userId, comment }) => {
    try {
        ...
        if (publication) {
            ...
            newComment = await newComment.populate('user', 'nombre'); // Popula el campo user con el nombre
