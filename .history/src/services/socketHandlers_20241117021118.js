...
socket.on('addComment', async ({ publicationId, userId, comment }) => {
    try {
        ...
        if (publication) {
            ...
            newComment = await newComment.populate('user', 'nombre').execPopulate(); // Popula el campo user con el nombre
            await newComment.populate('user', 'nombre').execPopulate(); // Popula el campo user con el nombre
