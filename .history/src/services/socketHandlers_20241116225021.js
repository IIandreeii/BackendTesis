...
// publicaciones
        ...
        socket.on('createPublication', async (formData) => {
            try {
                const { title, description, date, time, location, organizationName } = formData;
                const imageUrl = formData.image ? `/public/imagenes/${formData.image.filename}` : '';

                const charity = await Charity.findOne({ nombre: organizationName });
                if (!charity) {
