...
router.post('/chats', async (req, res) => {
  ...
  try {
    ...
    let chat = await Chat.findOne({
      participants: { $all: [{ _id: userId, type: userType }, { _id: receiverId, type: receiverType }] }
    }).populate('participants._id');  // Popula los detalles de los participantes

    if (!chat) {
      // Si no existe, crea un nuevo chat
      chat = new Chat({
        participants: [
          { _id: userId, type: userType },  // 'User' o 'Charity'
          { _id: receiverId, type: receiverType }  // 'User' o 'Charity'
        ],
        lastMessage: null,
      ...
      // Ahora, poblamos los participantes recién creados
      chat = await chat.populate('participants._id').execPopulate();
    }
