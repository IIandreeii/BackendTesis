...
router.get('/chats', async (req, res) => {
  ...
  try {
    ...
    const chatDetails = await Promise.all(chats.map(async (chat) => {
      ...
      // Poblar participante de tipo User
      const user = await User.findOne({ _id: userId }).select('nombre');
      if (user) {
        participantName = user.nombre;
      } else {
        // Poblar participante de tipo Charity
        const charity = await Charity.findOne({ _id: userId }).select('nombre');
        if (charity) {
          participantName = charity.nombre;
        }
      }

      return {
        id: chat._id,
        name: participantName,  // Nombre del participante
