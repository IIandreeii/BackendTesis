...
router.get('/chats', async (req, res) => {
  ...
  try {
    ...
    .populate({
      path: 'participants',
      // Primero, poblamos los usuarios de tipo User
      populate: {
        path: 'participants',
        model: 'User',
        select: 'nombre'
    ...
    .populate({
      path: 'participants',
      // Luego, poblamos los usuarios de tipo Charity
      populate: {
        path: 'participants',
        model: 'Charity',
        select: 'nombre'
    ...
    const chatDetails = chats.map(chat => {
      // Accedemos a los participantes y obtenemos sus nombres
      ...
      return {
        id: chat._id,
        name: participantNames.map(participant => participant.name).join(', '),  // Concatenamos los nombres
